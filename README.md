...
- 🎨 **Modern Web Interface**: Enjoy a clean, responsive Flask-based user interface.
...
2. **Upload CSV Files**
   ...
   - The uploaded files will appear in the data viewer with tabs for each file.

3. **Select Data and Generate Embeddings**

   - Choose the file you want to analyze from the dropdown.
   - Select the text columns for embedding generation using checkboxes.
...
6. **View Results**
   ...
   - **Generated SQL Query**: View the SQL query generated by the assistant.
   - **Query Results**: See the data retrieved by the query in a table format.
...
**🔥 Flask Migration**: The application has been migrated from Streamlit to Flask for improved performance, scalability, and customization options.

**Key Improvements:**
- ⚡ **Faster Performance**: Lightweight Flask backend
- 🎨 **Professional UI**: Bootstrap-based responsive design
- 📁 **Better File Management**: Efficient file-based storage system
- 🔗 **API Ready**: Easy to extend with REST API endpoints
- 🔧 **More Customizable**: Greater control over UI and functionality
...
1. **Flask Web Application**
   ...
   - Uses Flask sessions to maintain data across interactions.
   - Provides a responsive web interface with Bootstrap styling.
...
4. **DataFrameManager**
   ...
   - Uses file-based storage for efficient session management.
...
- **Flask Integration**: Async functionality preserved using threading for compatibility.
...
- **Flask Sessions**: Replaces Streamlit session state for data persistence.
- **File-based Storage**: Large DataFrames stored as files to avoid session size limits.
- **Efficient Memory Usage**: Only metadata stored in sessions, actual data loaded on demand.
